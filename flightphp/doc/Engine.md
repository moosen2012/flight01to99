...
_start(): 初始化框架并处理请求的入口方法。
_stop(): 停止框架并输出当前的响应。
_halt(int $code = 200, string $message = ''): 停止处理并返回给定的响应。
_route(string $pattern, callable $callback, bool $pass_route = false, string $alias = ''): 将 URL 路由到回调函数。
_group(string $pattern, callable $callback, array $group_middlewares = []): 将一组路由组合在一起，共享相同的前缀和中间件。
_post(string $pattern, callable $callback, bool $pass_route = false): 将 POST 请求的 URL 路由到回调函数。
_put(string $pattern, callable $callback, bool $pass_route = false): 将 PUT 请求的 URL 路由到回调函数。
_patch(string $pattern, callable $callback, bool $pass_route = false): 将 PATCH 请求的 URL 路由到回调函数。
_delete(string $pattern, callable $callback, bool $pass_route = false): 将 DELETE 请求的 URL 路由到回调函数。
_notFound(): 发送 HTTP 404 响应当找不到 URL 时。
_redirect(string $url, int $code = 303): 将当前请求重定向到另一个 URL。
模板渲染和视图处理：

_render(string $file, ?array $data = null, ?string $key = null): 渲染模板文件。
_json($data, int $code = 200, bool $encode = true, string $charset = 'utf-8', int $option = 0): 发送 JSON 响应。
_jsonp($data, string $param = 'jsonp', int $code = 200, bool $encode = true, string $charset = 'utf-8', int $option = 0): 发送 JSONP 响应。
_etag(string $id, string $type = 'strong'): 处理 ETag HTTP 缓存。
_lastModified(int $time): 处理最后修改的 HTTP 缓存。
_getUrl(string $alias, array $params = []): 根据别名获取 URL。
错误处理：

_error($e): 处理异常并发送 HTTP 500 响应。
_handleError(int $errno, string $errstr, string $errfile, int $errline): 自定义错误处理函数，将错误转换为异常。
变量处理和类注册：

__call(string $name, array $params): 处理类方法的调用。
get(?string $key = null): 获取变量。
set($key, $value = null): 设置变量。
has(string $key): 检查变量是否存在。
clear(?string $key = null): 清除变量。
path(string $dir): 添加类自动加载的路径。
register(string $name, string $class, array $params = [], ?callable $callback = null): 将类注册到框架方法。
unregister(string $methodName): 取消注册类到框架方法。
框架初始化：

init(): 初始化框架，注册默认组件和方法，并设置默认配置。
这个引擎类是 Flight 框架的核心，它提供了路由、请求处理、视图渲染和错误处理等核心功能，是整个框架的基础。